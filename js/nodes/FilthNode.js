...
export class FilthNode extends Serializable {
    ...
    Depth(val) {
        if (val !== undefined) {
            ...
            let add = Random.i.Generate(60);
            this.color.r = 170 - Math.min(GameMap.i.GetCorruption(this.pos), 40) + add;
            this.color.g = 150 - Math.min(GameMap.i.GetCorruption(this.pos), 80) + add;
        }
        return this.depth;
    }
    Position() {
