...
void UI.HandleKeyboard() {
    ...
    if (!currentMenu || !(currentMenu.Update(-1, -1, false, key) & KEYRESPOND)) {
        if (!textMode) {
            ...
            } else if (Game.DevMode() && key.c == keyMap["DevConsole"]) {
                ShowDevConsole();
            } else if (key.c == keyMap["TerrainOverlay"]) {
                if (Map.GetOverlayFlags() & TERRAIN_OVERLAY) Map.RemoveOverlay(TERRAIN_OVERLAY);
                else Map.AddOverlay(TERRAIN_OVERLAY);
            }
            ...
            if (key.vk == TCODK_UP) {
                ...
                drawCursor = true;
                mouseInput.x -= Game.CharWidth() * addition;
                mouseInput.cx -= addition;
                mouseInput.y += Game.CharHeight() * addition;
            ...
            if (key.vk >= TCODK_F1 && key.vk <= TCODK_F12) {
                if (ShiftPressed()) {
                    Game.SetMark(key.vk - TCODK_F1);
                } else {
                    Game.ReturnToMark(key.vk - TCODK_F1);
                }
            }
        } else {
    ...
    if (mouseInput.x < 0) {
        ...
        mouseInput.x = 0;
        mouseInput.cx = 0;
    } else if (mouseInput.cx >= Game.ScreenWidth()) {
        diffX += mouseInput.cx - (Game.ScreenWidth() - 1);
        mouseInput.cx = Game.ScreenWidth() - 1;
        mouseInput.x = (Game.ScreenWidth() - 1) * Game.CharWidth();
    }
    if (mouseInput.y < 0) {
        diffY += mouseInput.cy;
        mouseInput.y = 0;
        mouseInput.cy = 0;
    } else if (mouseInput.cy >= Game.ScreenHeight()) {
        diffY += mouseInput.cy - (Game.ScreenHeight() - 1);
        mouseInput.cy = Game.ScreenHeight() - 1;
        mouseInput.y = (Game.ScreenHeight() - 1) * Game.CharHeight();
    }

    Game.MoveCam(diffX, diffY);
}
...
void UI.HandleMouse() {
    ...
    if (TCODConsole.isWindowClosed()) Game.Exit();
    ...
    if (_state == UINORMAL) {
        HandleUnderCursor(Game.TileAt(mouseInput.x, mouseInput.y), & underCursor);
    }

    if (_state == UIPLACEMENT || _state == UIABPLACEMENT || _state == UIRECTPLACEMENT) {
        placeable = placementCallback(Game.TileAt(mouseInput.x, mouseInput.y), _blueprint);
    }

    if (_state == UIABPLACEMENT || _state == UIRECTPLACEMENT) {
        b = Game.TileAt(mouseInput.x, mouseInput.y);
    }
        ...
        if (draggingViewport) {
            ...
            if (!draggingPlacement) {
                ...
                if (menuResult & NOMENUHIT) {
                    menuResult = Announce.Update(mouseInput.cx, mouseInput.cy, true);
                    if (menuResult & NOMENUHIT) {
                        if (menuOpen) {
            ...
            if (menuResult & NOMENUHIT) {
                ...
                if (_state == UIPLACEMENT && placeable) {
                    callback(Game.TileAt(mouseInput.x, mouseInput.y));
                } else if (_state == UIABPLACEMENT && placeable) {
                    if (a.X() == 0) {
                        a = Game.TileAt(mouseInput.x, mouseInput.y);
    ...
    if (newMouseInput.lbutton && _state == UINORMAL) {
        Game.MoveCam(-(newMouseInput.dx / 3.0 f), -(newMouseInput.dy / 3.0 f));
...
void UI.Draw(TCODConsole * the_console) {
    ...
    Coordinate mouseTile(Game.TileAt(mouseInput.x, mouseInput.y));
    boost.shared_ptr < MapRenderer > renderer = Game.Renderer();
    ...
    if (_state == UINORMAL && (!menuOpen || (currentMenu.Update(mouseInput.cx, mouseInput.cy, false, NO_KEY) & NOMENUHIT)) &&
        ...
        (Announce.Update(mouseInput.cx, mouseInput.cy, false) & NOMENUHIT) &&
        !underCursor.empty()) {
        ...
        for (std.list < boost.weak_ptr < Entity > > .iterator ucit = underCursor.begin(); ucit != underCursor.end(); ++ucit) {
            if (boost.shared_ptr < Entity > entity = ucit.lock()) {
                Coordinate mouseLoc = Game.TileAt(mouseInput.x, mouseInput.y);
...
void UI.DrawTopBar(TCODConsole * the_console) {
    ...
    the_console.print(the_console.getWidth() / 2, 0, "w(%s)  -  %s  -  Orcs: %d   Goblins: %d  -  Year %d, %s  FPS: %d", Map.GetWindAbbreviation().c_str(),
        ...
        Game.GetAge(),
        Game.SeasonToString(Game.CurrentSeason()).c_str(),
        TCODSystem.getFps());
    ...
    if (keyHelpTextColor > 0) {
        ...
        x += DrawShortcutHelp(the_console, x, 3, "Workshops");
        x += DrawShortcutHelp(the_console, x, 3, "Orders");
        x += DrawShortcutHelp(the_console, x, 3, "Furniture");
        x += DrawShortcutHelp(the_console, x, 3, "StockManager");
        x += DrawShortcutHelp(the_console, x, 3, "Squads");
...
void UI.ChooseConstruct(ConstructionType construct, UIState state) {
    UI.SetCallback(boost.bind(Game.PlaceConstruction, _1, construct));
    UI.SetPlacementCallback(boost.bind(Game.CheckPlacement, _1, _2, Construction.Presets[construct].tileReqs));
    UI.blueprint(Construction.Blueprint(construct));
    UI.state(state);
    Game.Renderer().SetCursorMode(Cursor_Construct);
    UI.HideMenu();
...
void UI.ChooseStockpile(ConstructionType stockpile) {
    ...
    if (Construction.Presets[stockpile].tags[STOCKPILE]) {
        UI.SetCallback(boost.bind(Game.PlaceStockpile, _1, _1, stockpile, stockpileSymbol));
        UI.state(UIPLACEMENT);
    } else {
        UI.SetRectCallback(boost.bind(Game.PlaceStockpile, _1, _2, stockpile, stockpileSymbol));
        UI.state(UIRECTPLACEMENT);
    }
    UI.SetPlacementCallback(boost.bind(Game.CheckPlacement, _1, _2, Construction.Presets[stockpile].tileReqs));
    UI.blueprint(Construction.Blueprint(stockpile));
    Game.Renderer().SetCursorMode(Cursor_Stockpile);
    UI.HideMenu();
...
void UI.ChooseTreeFelling() {
    UI.state(UIRECTPLACEMENT);
    UI.SetRectCallback(boost.bind(Game.FellTree, _1, _2));
    UI.SetPlacementCallback(boost.bind(Game.CheckTree, _1, _2));
    UI.blueprint(Coordinate(1, 1));
    Game.Renderer().SetCursorMode(Cursor_TreeFelling);
    UI.HideMenu();
...
void UI.ChoosePlantHarvest() {
    UI.state(UIRECTPLACEMENT);
    UI.SetRectCallback(boost.bind(Game.HarvestWildPlant, _1, _2));
    UI.SetPlacementCallback(boost.bind(Game.CheckTree, _1, _2));
    UI.blueprint(Coordinate(1, 1));
    Game.Renderer().SetCursorMode(Cursor_Harvest);
    UI.HideMenu();
...
void UI.ChooseOrderTargetCoordinate(boost.shared_ptr < Squad > squad, Order order) {
    ...
    UI.SetCallback(boost.bind(Game.SetSquadTargetCoordinate, order, _1, squad, autoClose));
    UI.SetPlacementCallback(boost.bind(Game.CheckTree, _1, Coordinate(1, 1)));
    UI.blueprint(Coordinate(1, 1));
    Game.Renderer().SetCursorMode(Cursor_Order);
}

void UI.ChooseOrderTargetEntity(boost.shared_ptr < Squad > squad, Order order) {
    UI.state(UIPLACEMENT);
    UI.SetCallback(boost.bind(Game.SetSquadTargetEntity, order, _1, squad));
    UI.SetPlacementCallback(boost.bind(Game.CheckTree, _1, Coordinate(1, 1)));
    UI.blueprint(Coordinate(1, 1));
    Game.Renderer().SetCursorMode(Cursor_Order);
}
...
void UI.ChooseDesignateTree() {
    UI.state(UIRECTPLACEMENT);
    UI.SetRectCallback(boost.bind(Game.DesignateTree, _1, _2));
    UI.SetPlacementCallback(boost.bind(Game.CheckTree, _1, _2));
    UI.blueprint(Coordinate(1, 1));
    Game.Renderer().SetCursorMode(Cursor_Tree);
    UI.HideMenu();
...
void UI.ChooseDismantle() {
    UI.state(UIRECTPLACEMENT);
    UI.SetRectCallback(boost.bind(Game.DismantleConstruction, _1, _2));
    UI.SetPlacementCallback(boost.bind(Game.CheckTree, _1, _2));
    UI.blueprint(Coordinate(1, 1));
    Game.Renderer().SetCursorMode(Cursor_Dismantle);
    UI.HideMenu();
...
void UI.ChooseUndesignate() {
    UI.state(UIRECTPLACEMENT);
    UI.SetRectCallback(boost.bind(Game.Undesignate, _1, _2));
    UI.SetPlacementCallback(boost.bind(Game.CheckTree, _1, _2));
    UI.blueprint(Coordinate(1, 1));
    Game.Renderer().SetCursorMode(Cursor_Undesignate);
    UI.HideMenu();
...
void UI.ChooseDesignateBog() {
    UI.state(UIRECTPLACEMENT);
    UI.SetRectCallback(boost.bind(Game.DesignateBog, _1, _2));
    UI.SetPlacementCallback(boost.bind(Game.CheckTileType, TILEBOG, _1, _2));
    UI.blueprint(Coordinate(1, 1));
    Game.Renderer().SetCursorMode(Cursor_Bog);
    UI.HideMenu();
...
boost.weak_ptr < Entity > UI.GetEntity(const Coordinate & pos) {
    if (pos.X() >= 0 && pos.X() < Map.Width() && pos.Y() >= 0 && pos.Y() < Map.Height()) {
        std.set < int > * npcList = Map.NPCList(pos);
        if (!npcList.empty()) return Game.GetNPC(( * npcList.begin()));

        std.set < int > * itemList = Map.ItemList(pos);
        if (!itemList.empty()) {
            std.set < boost.weak_ptr < Item > > .iterator itemi = Game.freeItems.find(Game.itemList[ * itemList.begin()]);
            if (itemi != Game.freeItems.end()) {
                return *itemi;
            }
        }

        int entity = Map.GetNatureObject(pos);
        if (entity > -1) return (Game.natureList[entity]);

        entity = Map.GetConstruction(pos);
        if (entity > -1) return Game.GetConstruction(entity);
    }
    ...
    if (Map.IsInside(pos)) {
        ...
        if (!npcList.empty()) {
            for (std.set < int > .iterator npci = npcList.begin(); npci != npcList.end(); ++npci) {
                result.push_back(Game.GetNPC( * npci));
            }
        }
        ...
        if (!itemList.empty()) {
            for (std.set < int > .iterator itemi = itemList.begin(); itemi != itemList.end(); ++itemi) {
                result.push_back(Game.itemList[ * itemi]);
            }
        }

        int entity = Map.GetNatureObject(pos);
        if (entity > -1) {
            result.push_back(Game.natureList[entity]);
        }

        entity = Map.GetConstruction(pos);
        if (entity > -1) {
            result.push_back(Game.GetConstruction(entity));
        }
    }
}
...
void UI.ChooseCreateNPC() {
    ...
    if (npc >= 0) {
        UI.state(UIPLACEMENT);
        UI.SetCallback(boost.bind( & Game.CreateNPC, Game.Inst(), _1, NPCType(npc)));
        UI.SetPlacementCallback(boost.bind(Game.CheckTree, _1, Coordinate(1, 1)));
        UI.blueprint(Coordinate(1, 1));
        Game.Renderer().SetCursorMode(NPC.Presets[npc]);
    }
}
...
void UI.ChooseCreateItem() {
    ...
    if (item >= 0) {
        UI.state(UIPLACEMENT);
        UI.SetCallback(boost.bind( & Game.CreateItem, Game.Inst(), _1, ItemType(item), false,
            0, std.vector < boost.weak_ptr < Item > > (), boost.shared_ptr < Container > ()));
        UI.SetPlacementCallback(boost.bind(Game.CheckTree, _1, Coordinate(1, 1)));
        UI.blueprint(Coordinate(1, 1));
        Game.Renderer().SetCursorMode(Item.Presets[item]);
    }
...
void UI.ChooseDig() {
    UI.state(UIRECTPLACEMENT);
    UI.SetRectCallback(boost.bind(Game.Dig, _1, _2));
    UI.SetPlacementCallback(boost.bind(Game.CheckPlacement, _1, _2, std.set < TileType > ()));
    UI.blueprint(Coordinate(1, 1));
    Game.Renderer().SetCursorMode(Cursor_Dig);
    UI.HideMenu();
...
void UI.ChooseNaturify() {
    for (int i = 0; i < 10000; ++i) {
        Map.Naturify(Random.ChooseInExtent(Map.Extent()));
    }
}
...
void UI.ChooseChangeTerritory(bool add) {
    ...
    UI.SetPlacementCallback(boost.bind(Game.CheckTree, _1, _2));
    UI.blueprint(Coordinate(1, 1));
    Game.Renderer().SetCursorMode(add ? Cursor_AddTerritory : Cursor_RemoveTerritory);
    UI.HideMenu();
...
void UI.ChooseGatherItems() {
    UI.state(UIRECTPLACEMENT);
    UI.SetRectCallback(boost.bind( & Game.GatherItems, Game.Inst(), _1, _2));
    UI.SetPlacementCallback(boost.bind(Game.CheckTree, _1, _2));
    UI.blueprint(Coordinate(1, 1));
    Game.Renderer().SetCursorMode(Cursor_Gather);
    UI.HideMenu();
...
void UI.ChooseNormalPlacement(boost.function < void(Coordinate) > callback, boost.function < bool(Coordinate, Coordinate) > placement, int cursor, std.string optionalTooltip) {
    ...
    UI.blueprint(Coordinate(1, 1));
    Game.Renderer().SetCursorMode(cursor);
