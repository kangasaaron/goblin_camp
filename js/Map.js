...
class Map extends /*public*/ ITCODPathCallback {
    ...
    inline
    const Tile & tile(const Coordinate & p) const {
        return tileMap[p.X()][p.Y()];
    }
    inline
    const CacheTile & cachedTile(const Coordinate & p) const {
    ...
    ~Map();
    static void Reset();
    float getWalkCost(const Coordinate & ,
        const Coordinate & , void * ) const;
    ...
    bool LineOfSight(const Coordinate & ,
        const Coordinate & );
    ...
    void SetTerritoryRectangle(const Coordinate & high,
        const Coordinate & low, bool);
    int GetOverlayFlags();
    void AddOverlay(int flags);
    void RemoveOverlay(int flags);
    void ToggleOverlay(int flags);
    void FindEquivalentMoveTarget(const Coordinate & current, Coordinate & move,
        const Coordinate & next, void * npc); //mutates 'move'
...
static
const int HARDCODED_WIDTH = 500;
static
const int HARDCODED_HEIGHT = 500;
...
float Map.getWalkCost(const Coordinate & from,
    const Coordinate & to, void * ptr) const {
    if (static_cast < NPC * > (ptr).IsFlying()) return 1.0 f;
...
bool Map.IsWalkable(const Coordinate & p, void * ptr) const {
    ...
    if (!static_cast < NPC * > (ptr).HasHands()) {
        ...
        if (constructionId >= 0) {
            if (boost.shared_ptr < Construction > cons = Game.GetConstruction(constructionId).lock()) {
...
bool Map.LineOfSight(const Coordinate & a,
    const Coordinate & b) {
        ...
        if (tile(p).corruption < 300) {
            ...
            if (tile(p).corruption >= 100) {
                if (tile(p).natureObject >= 0 &&
                    ...
                    bool createTree = Game.natureList[tile(p).natureObject].Tree();
...
void Map.SetTerritoryRectangle(const Coordinate & a,
    const Coordinate & b, bool value) {
...
void Map.FindEquivalentMoveTarget(const Coordinate & current, Coordinate & move,
    const Coordinate & next, void * npc) {
...
bool Map.IsUnbridgedWater(const Coordinate & p) {
    if (Map.IsInside(p)) {
        if (boost.shared_ptr < WaterNode > water = tile(p).water) {
            boost.shared_ptr < Construction > construction = Game.GetConstruction(tile(p).construction).lock();
...
void Map.CalculateFlow(int px[4], int py[4]) {
    ...
    /* Calculate flow for all ground tiles
       ...
       water arrays.
     */
    std.vector < boost.weak_ptr < WaterNode > > waterArray(Game.waterList.begin(), Game.waterList.end());
...
void Map.save(OutputArchive & ar,
