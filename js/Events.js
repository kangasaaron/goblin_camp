...
void Events.Update(bool safe) {
    ...
    if (static_cast < int > (existingImmigrants.size()) < Game.OrcCount() / 7 &&
        Random.Generate(UPDATES_PER_SECOND * 60 * 30) == 0) {
        SpawnImmigrants();
    }

    Season cSeason = Game.CurrentSeason();
    ...
    if (!possibleMonsters.empty()) {
        ...
        if (hostileSpawnCount > 1)
            msg = (boost.format("%s have been sighted outside your %s!") %
                NPC.Presets[monsterType].plural % Camp.GetName()).str();
        else msg = (boost.format("A %s has been sighted outside your %s!") %
            NPC.Presets[monsterType].name % Camp.GetName()).str();

        Coordinate a, b;
        GenerateEdgeCoordinates(map, a, b);

        Game.CreateNPCs(hostileSpawnCount, monsterType, a, b);
        Announce.AddMsg(msg, TCODColor.red, Coordinate((a.X() + b.X()) / 2, (a.Y() + b.Y()) / 2));
        timeSinceHostileSpawn = 0;
...
void Events.SpawnBenignFauna() {
    if (peacefulAnimals.size() > 0 && Game.PeacefulFaunaCount() < 20) {
...
void Events.SpawnImmigrants() {
    ...
    if (!possibleImmigrants.empty()) {
        ...
        for (int i = 0; i < spawnCount; ++i) {
            int npcUid = Game.CreateNPC(Random.ChooseInRectangle(a, b), monsterType);
            existingImmigrants.push_back(Game.GetNPC(npcUid));
        }

        Announce.AddMsg(msg, TCODColor(0, 150, 255), (a + b) / 2);
    }
}
...
void Events.SpawnMigratingAnimals() {
    if (!migratingAnimals.empty()) {
        ...
        int tuid = Game.CreateNPC(a, monsterType);
        migrationSpawnCount--;
        if (DEBUG) {
            std.cout << "Migration spawning " << migrationSpawnCount << " " << monsterType << "\n";
        } /*#endif*/

        NPC * firstNPC = Game.GetNPC(tuid).get();
        int x, y;
        int halfMapWidth, halfMapHeight;
        halfMapWidth = Map.Width() / 2;
        halfMapHeight = Map.Height() / 2;
        ...
        std.vector < int > uids = Game.CreateNPCs(migrationSpawnCount, monsterType, a, b);

        for (std.vector < int > .iterator uidi = uids.begin(); uidi != uids.end(); uidi++) {
            boost.shared_ptr < NPC > ptr = Game.GetNPC( * uidi);
            if (!ptr) continue;
        ...
        for (std.vector < NPC * > .iterator mgrnt = migrants.begin(); mgrnt != migrants.end(); mgrnt++) {
        ...
        Announce.AddMsg(msg, TCODColor.green, (a + b) / 2);
        if (DEBUG) {
