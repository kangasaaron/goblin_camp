...
export class Item extends Entity {
    ...
    constructor(startPos = Coordinate.zero, typeval = -1, owner = -1, components = []) {
        super();
    ...
    destructor() {
        ...
        if (this.faction === Faction.PLAYERFACTION) {
            this.StockManager.UpdateQuantity(this.type, -1);
        }
    }
        ...
        if (this.components.length > 0) {
            for (let i = 0; i < this.components.length; ++i) {
                if (this.components[i].lock()) {
                    this.color = TCODColor.lerp(this.color, this.components[i].lock().Color(), 0.35);
    ...
    Draw(upleft, the_console) {
        ...
        if (screenx >= 0 && screenx < the_console.getWidth() && screeny >= 0 && screeny < the_console.getHeight()) {
            the_console.putCharEx(screenx, screeny, this.graphic, this.color, this.map.GetBackColor(this.pos));
        }
    }
    Type() {
        return this.type;
    ...
    Position(p) {
        if (typeof p !== "undefined") {
            if (!this.map.IsInside(p)) {
                if (!this.internal && !this.container.lock()) this.map.ItemList(this.pos).erase(this.uid);
                this.pos = p;
                if (!this.internal && !this.container.lock()) this.map.ItemList(this.pos).insert(this.uid);
            }
        }
    ...
    Reserve(value) {
        ...
        if (!this.reserved && !this.container.lock() && !this.attemptedStore) {
            ...
            this.Game.i.StockpileItem(this);
        }
    }
    ...
    PutInContainer(con) {
        ...
        this.Game.i.ItemContained(this, !!this.container.lock());
    ...
    SetFaction(val) {
        if (val === Faction.PLAYERFACTION && this.faction !== Faction.PLAYERFACTION) { //Transferred to player
            this.StockManager.UpdateQuantity(this.type, 1);
        } else if (val !== Faction.PLAYERFACTION && this.faction === Faction.PLAYERFACTION) { //Transferred from player
            this.StockManager.UpdateQuantity(this.type, -1);
        }
        this.faction = val;
    }
    ...
    SetVelocity(speed) {
        this.velocity = speed;
        if (speed > 0) {
            this.Game.i.flyingItems.push(this);
            return;
        }

        //The item has moved before but has now stopped
        this.Game.i.stoppedItems.push(this);

        if (this.map.IsWalkable(this.pos)) return;
        ...
        for (let radius = 1; radius < 10; ++radius) {
            ...
            for (let ix = this.pos.X() - radius; ix <= this.pos.X() + radius; ++ix) {
                for (let iy = this.pos.Y() - radius; iy <= this.pos.Y() + radius; ++iy) {
                    ...
                    if (!this.map.IsWalkable(p)) continue;
                    this.Position(p);
                    return;
                }
            }
    ...
    Impact(speedChange) {
        ...
        if (speedChange >= 10 && this.Random.i.Generate(9) < 7)
        ...
        if (this.condition === 0) { //Note that condition < 0 means that it is not damaged by impacts
            ...
            this.Game.i.CreateItem(this.Position(), Item.StringToItemType("debris"), false, -1, component);
            //Game.i.Update removes all condition==0 items in the stopped items list, which is where this item will be
        }
    }
    CollisionDetection() {
        let t = this.flightPath[this.flightPath.length - 1].coord;

        if (this.map.BlocksWater(t) || !this.map.IsWalkable(t)) { //We've hit an obstacle
            let attack = this.GetAttack();
            if (this.map.GetConstruction(t) > -1) {
                let construct = this.Game.i.GetConstruction(this.map.GetConstruction(t)).lock();
                if (construct) {
        ...
        if (this.map.NPCList(t).length > 0) { //Hit a creature
            if (this.Random.i.Generate(Math.max(1, this.flightPath[this.flightPath - 1].height) - 1) < (2 + this.map.NPCList(t).length)) {
                let attack = this.GetAttack();
                let npc = this.Game.i.GetNPC(this.map.NPCList(t)[0]);
                npc.Damage(attack);
