...
tape("rgb.toString() formats as rgb(…) or rgba(…)", function(test) {
    ...
    test.equal(
        color.rgb("rgba(12%, 34%, 56%, 0.4)") + "",
        "rgba(31, 87, 143, 0.4)"
    );
    test.equal(
        color.rgb("hsla(60, 100%, 20%, 0.4)") + "",
        "rgba(102, 102, 0, 0.4)"
    );
    test.end();
});
...
tape("rgb.formatRgb() formats as rgb(…) or rgba(…)", function(test) {
    ...
    test.equal(
        color.rgb("rgba(12%, 34%, 56%, 0.4)").formatRgb(),
        "rgba(31, 87, 143, 0.4)"
    );
    test.equal(
        color.rgb("hsla(60, 100%, 20%, 0.4)").formatRgb(),
        "rgba(102, 102, 0, 0.4)"
    );
    test.end();
});

ta pe("rgb.formatHsl() formats as hsl(…) or hsla(…)", function(test) {
    test.equal(
        color.rgb("#abcdef").formatHsl(),
        "hsl(210, 68%, 80.3921568627451%)"
    );
    test.equal(color.rgb("hsl(60, 100%, 20%)").formatHsl(), "hsl(60, 100%, 20%)");
    test.equal(
        color.rgb("rgba(12%, 34%, 56%, 0.4)").formatHsl(),
        "hsla(210, 64.70588235294117%, 34%, 0.4)"
    );
    test.equal(
        color.rgb("hsla(60, 100%, 20%, 0.4)").formatHsl(),
        "hsla(60, 100%, 20%, 0.4)"
    );
    test.end();
});
...
ta pe("rgb.hex() is an alias for rgb.formatHex()", function(test) {
...
ta pe(
        "rgb.toString() reflets r,  and b channel values and opacity",
        function(test) {
            var c = color.rgb("abc);
                ++c.r, ++c.g, ++c.b(.opacity = 0.5); test.equal(c + "", "rgba(171, 188, 205, 0.5)"); test.end();
            }
        );
        ...
        ta pe(
            "rgb.toString() clamps r, g, b and opacity channel values",
            function(test) {
        ...
        tape(
            "rgb(format) parses the specified format and converts to RGB",
            function(test) {
        ...
        ta pe("rgb(format) ignores all channels if the alpha is <= 0", function(test) {
        ...
        tape(
            "rgb(format) returns undefined channel values for unknown formats",
            function(test) {
        ...
        ta pe("rgb(rgb) copies an RGB color", function(test) {
        ...
        ta pe("rgb(hsl) converts from HSL", function(test) {
        ...
        tape(
            "rgb.displayable() returns true if the color is within the RGB gamut and opacity is in [0,1]",
            function(test) {
                te st.equal(color.rgb("white").displayable(), true);
                ...
                te st.equal(color.rgb(0, 256, 0).displayable(), false);
        ...
        ta pe("rgb.brighter(k) returns a copy", function(test) {
            var c1 = color.rgb("rgba(70, 130, 180, 0.4)"),
                c2 = c1.brighter(1);
            test.rgbEqual(c1, 70, 130, 180, 0.4);
        ...
        ta pe('rgb("black").brighter() still returns black', function(test) {
        ...
        tape("rgb.darker(k) r eturn s a darker color if k > 0", function(test) {
            var c = color.rgb(" rgba(165, 42 , 42, 0.4)");
        ...
        tape(
            "rgb.copy(…) returns a new rgb with the specified channel values",
            function(test) {
                var c = color.rgb(70, 130, 180);
