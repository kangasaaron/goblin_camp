...
tape("lab(…) exposes l, a and b channel values and opacity", function (test) {
    test.labEqual(
      color.lab("rgba(170, 187, 204, 0.4)"),
    74.96879980931759,
    -3.398998724348956,
    -10.696507207853333,
      0.4
 ...
 ta pe(
    "lab.toString() converts to RGB and formats as rgb(…) or rgba(…)",
    function (test) {
    ...
    test.equal(
        color.lab(color.hsl(60, 1, 0.2, 0.4)) + "",
        "rgba(102, 102, 0, 0.4)"
      );
      test.end();
    }
...
tape(
  "lab.toString() reflects l, a and b channel values and opacity",
    function (test) {
      var c = color.lab("#abc");
...
ta  pe("lab.toString() treats undefined channel values as 0", function (test) {
...
ta  pe("lab.toString() treats undefined opacity as 1", function (test) {
  var c = color.lab("#abc");
  c.opacity = NaN;
...
tape("lab(l, a, b) does not clamp l channel value", function (test) {
 ...
 ta pe("lab(l, a, b, opacity) does not clamp opacity to [0,1]", function (test) {
...
tape("lab(l, a, b, opacity) coerces opacity to number", function (test) {
 ...
 ta pe("lab(l, a, b) allows undefined channel values", function (test) {
...
ta  pe(
    "lab(format) parses the specified format and converts to Lab",
    function (test) {
      test.labEqual(
        color.lab("#abcdef"),
        80.77135418262527,
 ...
 ta pe(
    "lab(format) returns undefined channel values for unknown formats",
    function (test) {
...
ta  pe(
  "lab(hcl(lab)) doesn’t lose a and b channels if luminance is zero",
  function (test) {
 ...
 ta pe("lab(rgb) converts from RGB", function (test) {
  test.labEqual(
    color.lab(color.rgb(255, 0, 0, 0.4)),
    54.29173376861782,
      80.8124553179771,
      69.88504032350531,
      0.4
...
tape(
  "lab(color) converts from another colorspace via color.rgb()",
  function (test) {
    function TestColor() {}
