...
export class Container extends Item {
    ...
    GetTooltip(x, y, tooltip) {
        ...
        tooltip.AddEntry(
            new TooltipEntry(
                (
                    (((boost.format("%s - %d items (%d/%d)") % this.name) % this.size()) %
                        capacityUsed) %
    ...
    TranslateContainerListeners() {
        ...
        for (let i = 0; i < this.listenersAsUids.length; ++i) {
            let cons = Game.GetConstruction(this.listenersAsUids[i]);
    ...
    AddWater(amount) {
        ...
        for (let i = 0; i < amount; ++i) {
            let waterUid = Game.CreateItem(
                this.Position(),
                Item.StringToItemType("Water")
            );
            let waterItem = Game.GetItem(waterUid).lock();
