...
class Spell extends /*public*/ Entity {
...
void Spell.Draw(Coordinate upleft, TCODConsole * the_console) {
                        ...
                        if (Map.BlocksWater(t) || !Map.IsWalkable(t)) { //We've hit an obstacle
                            ...
                            for (std.list < Attack > .iterator attacki = attacks.begin(); attacki != attacks.end(); ++attacki) {
                                if (attacki.Type() == DAMAGE_FIRE) {
                                    ...
                                    obstacle it hit */
                                    Game.CreateFire(flightPath.back().coord, 5);
                                    break;
                                }
                            }
                            Impact(velocity);
...
class SpellListener extends /*public*/ ITCODParserListener {
    int spellIndex;
    ...
    bool parserProperty(TCODParser * parser,
        ...
        if (boost.iequals(name, "name")) { Spell.Presets[spellIndex].name = value.s; } else if (boost.iequals(name, "speed")) { Spell.Presets[spellIndex].speed = value.i; } else if (boost.iequals(name, "col")) { Spell.Presets[spellIndex].color = value.col; } else if (boost.iequals(name, "graphic")) { Spell.Presets[spellIndex].graphic = value.i; } else if (boost.iequals(name, "fallbackGraphicsSet")) { Spell.Presets[spellIndex].fallbackGraphicsSet = value.s; } else if (boost.iequals(name, "type")) {
...
void Spell.save(OutputArchive & ar,
...
void Spell.load(InputArchive & ar,
