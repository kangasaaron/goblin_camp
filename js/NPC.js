...
void NPC.HandleHunger() {
	...
	if (!found) {
		...
		if (!item.lock()) {
			item = Game.Inst().FindItemByCategoryFromStockpiles(Item.StringToItemCategory("Food"), Position(), MOSTDECAYED | AVOIDGARBAGE);
		}
...
void NPC.UpdateStatusEffects() {
	...
	if (factionPtr.IsFriendsWith(PLAYERFACTION)) effectiveResistances[DISEASE_RES] = Math.max(0, effectiveResistances[DISEASE_RES] - Camp.Inst().GetDiseaseModifier());
