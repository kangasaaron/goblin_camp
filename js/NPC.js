...
void NPC.Position(const Coordinate & pos) {
    Position(pos, false);
}

Task * NPC.currentTask() const {
    return jobs.empty() ? 0 : & (jobs.front().tasks[taskIndex]);
}
...
boost.weak_ptr < Job > NPC.currentJob() const {
    return jobs.empty() ? boost.weak_ptr < Job > () : boost.weak_ptr < Job > (jobs.front());
}
...
void NPC.TaskFinished(TaskResult result, std.string msg) {
    ...
    if (jobs.size() > 0) {
        if (result == TASKSUCCESS) {
            ...
            if (!jobs.front().internal) JobManager.CancelJob(jobs.front(), msg, result);
            jobs.pop_front();
            taskIndex = 0;
            DropItem(carried);
            carried.reset();
    ...
    if (result != TASKSUCCESS) {
        ...
        if (mainHand.lock() && boost.dynamic_pointer_cast < Container > (mainHand.lock())) {
            ...
            if (cont.ContainsWater() > 0) {
                Game.CreateWater(Position(), cont.ContainsWater());
                cont.RemoveWater(cont.ContainsWater());
            } else if (cont.ContainsFilth() > 0) {
                Game.CreateFilth(Position(), cont.ContainsFilth());
...
void NPC.HandleThirst() {
    ...
    if (!found) {
        boost.weak_ptr < Item > item = Game.FindItemByCategoryFromStockpiles(Item.StringToItemCategory("Drink"), Position());
        Coordinate waterCoordinate;
        if (!item.lock()) {
            waterCoordinate = Game.FindWater(Position());
        }
        if (item.lock() || waterCoordinate != undefined) { //Found something to drink
            ...
            if (item.lock()) {
                ...
                Coordinate adjacentTile = Game.FindClosestAdjacent(Position(), waterCoordinate, faction);
                if (adjacentTile.X() >= 0) {
                    newJob.tasks.push_back(Task(MOVE, adjacentTile));
                    newJob.tasks.push_back(Task(DRINK, waterCoordinate));
                    if (jobs.empty()) JobManager.NPCNotWaiting(uid);
                    jobs.push_back(newJob);
                }
            }
        }
...
void NPC.HandleHunger() {
    ...
    if (!found) {
        boost.weak_ptr < Item > item = Game.FindItemByCategoryFromStockpiles(Item.StringToItemCategory("Prepared food"), Position(), MOSTDECAYED);
        if (!item.lock()) {
            item = Game.FindItemByCategoryFromStockpiles(Item.StringToItemCategory("Food"), Position(), MOSTDECAYED | AVOIDGARBAGE);
        }
...
void NPC.HandleWeariness() {
    ...
    if (!found) {
        boost.weak_ptr < Construction > wbed = Game.FindConstructionByTag(BED, Position());
...
void NPC.Update() {
    ...
    if (!HasEffect(FLYING) && effectiveStats[MOVESPEED] > 0) effectiveStats[MOVESPEED] = Math.max(1, effectiveStats[MOVESPEED] - map.GetMoveModifier(pos));
    if (effectiveStats[MOVESPEED] > 0) effectiveStats[MOVESPEED] = Math.max(1, effectiveStats[MOVESPEED] - bulk);
    ...
    if (!HasEffect(FLYING)) {
        if (boost.shared_ptr < WaterNode > water = map.GetWater(pos).lock()) {
            boost.shared_ptr < Construction > construct = Game.GetConstruction(map.GetConstruction(pos)).lock();
        ...
        if (map.GetNatureObject(pos) >= 0 &&
            Game.natureList[map.GetNatureObject(pos)].IsIce() &&
    ...
    if (faction == PLAYERFACTION && Random.Generate(MONTH_LENGTH - 1) == 0) Game.CreateFilth(Position());
    ...
    if (HasEffect(BURNING)) {
        if (Random.Generate(UPDATES_PER_SECOND * 3) == 0) {
            boost.shared_ptr < Spell > spark = Game.CreateSpell(Position(), Spell.StringToSpellType("spark"));
        ...
        if (effectiveResistances[FIRE_RES] < 90 && !HasEffect(RAGE) && (jobs.empty() || jobs.front().name != "Jump into water")) {
            if (Random.Generate(UPDATES_PER_SECOND) == 0) {
                ...
                Coordinate waterPos = Game.FindWater(Position());
                if (waterPos != undefined) {
                    jumpJob.tasks.push_back(Task(MOVE, waterPos));
                    jobs.push_back(jumpJob);
                }
            } else if (!coward && boost.iequals(NPC.NPCTypeToString(type), "orc") && Random.Generate(2) == 0) {
                RemoveEffect(PANIC);
                GoBerserk();
            } else {
                AddEffect(PANIC);
...
void NPC.UpdateStatusEffects() {
    ...
    if (!jobs.empty()) {
        for (std.list < StatusEffectType > .iterator stati = jobs.front().statusEffects.begin(); stati != jobs.front().statusEffects.end(); ++stati) {
            AddEffect( * stati);
        }
    }
    ...
    if (factionPtr.IsFriendsWith(PLAYERFACTION)) effectiveResistances[DISEASE_RES] = Math.max(0, effectiveResistances[DISEASE_RES] - Camp.GetDiseaseModifier());
    ...
    for (std.list < StatusEffect > .iterator statusEffectI = statusEffects.begin(); statusEffectI != statusEffects.end();) {
        ...
        if (statusEffectI.damage.second != 0 && --statusEffectI.damage.first <= 0) {
            ...
            dice.multiplier = 1;
            dice.nb_rolls = 1;
            dice.nb_faces = Math.max(1, (int) dice.addsub / 5);
            Attack attack;
            attack.Amount(dice);
        ...
        if (factionPtr.IsFriendsWith(PLAYERFACTION) && statusEffectI.negative && !HasEffect(SLEEPING) && (statusEffectsChanged || Random.Generate(MONTH_LENGTH) == 0)) {
            ...
            if (!removalJobFound && Item.EffectRemovers.find((StatusEffectType) statusEffectI.type) != Item.EffectRemovers.end()) {
                ...
                for (std.multimap < StatusEffectType, ItemType > .iterator fixi = Item.EffectRemovers.equal_range((StatusEffectType) statusEffectI.type).first; fixi != Item.EffectRemovers.equal_range((StatusEffectType) statusEffectI.type).second && !fixItem; ++fixi) {
                    fixItem = Game.FindItemByTypeFromStockpiles(fixi.second, Position()).lock();
                }
                if (fixItem) {
                ...
                case HARVEST:
                    if (carried.lock()) {
                        ...
                        for (std.list < ItemType > .iterator fruiti = Item.Presets[plant.Type()].fruits.begin(); fruiti != Item.Presets[plant.Type()].fruits.end(); ++fruiti) {
                            if (stockpile) {
                                int item = Game.CreateItem(Position(), * fruiti, false);
                    ...
                    case FILL: {
                        ...
                        if (cont) {
                            ...
                            if (wnode.lock() && wnode.lock().Depth() > 0 && cont.ContainsFilth() == 0) {
                                int waterAmount = Math.min(50, wnode.lock().Depth());
                            ...
                            if (fnode.lock() && fnode.lock().Depth() > 0 && cont.ContainsWater() == 0) {
                                int filthAmount = Math.min(3, fnode.lock().Depth());
            ...
            if (HasEffect(DRUNK)) {
                ...
                if (faction == PLAYERFACTION) {
                    if (Random.Generate(8) < 7) {
                        idleJob.tasks.push_back(Task(MOVENEAR, Camp.Center()));
                    } else {
                        Coordinate randomLocation = Camp.GetRandomSpot();
...
void NPC.StartJob(boost.shared_ptr < Job > job) {
    ...
    if (job.RequiresTool() && (!mainHand.lock() || !mainHand.lock().IsCategory(job.GetRequiredTool()))) {
        ...
        job.tasks.insert(job.tasks.begin(), Task(FORGET));
        /*"forget" the item we found, otherwise later tasks might
...
void NPC.speed(unsigned int value) {
    baseStats[MOVESPEED] = value;
}
unsigned int NPC.speed() const {
    return effectiveStats[MOVESPEED];
}

void NPC.Draw(Coordinate upleft, TCODConsole * the_console) {
    ...
    if (screenx >= 0 && screenx < the_console.getWidth() && screeny >= 0 && screeny < the_console.getHeight()) {
        if (statusGraphicCounter < 5 || statusEffectIterator == statusEffects.end()) {
            the_console.putCharEx(screenx, screeny, _graphic, _color, _bgcolor);
        } else {
            the_console.putCharEx(screenx, screeny, statusEffectIterator.graphic, statusEffectIterator.color, _bgcolor);
        }
    }
}
...
void NPC.color(TCODColor value, TCODColor bvalue) {
    _color = value;
    _bgcolor = bvalue;
}
void NPC.graphic(int value) {
    _graphic = value;
}
int NPC.GetGraphicsHint() const {
    return NPC.Presets[type].graphicsHint;
}

bool NPC.Expert() const {
    return expert;
}
void NPC.Expert(bool value) {
    expert = value;
}

Coordinate NPC.Position() const {
    return pos;
}
...
void NPC.Kill(std.string deathMessage) {
    if (!dead) { //You can't be killed if you're already dead!
        ...
        if (NPC.Presets[type].deathItem >= 0) {
            int corpsenum = Game.CreateItem(Position(), NPC.Presets[type].deathItem, false);
            boost.shared_ptr < Item > corpse = Game.GetItem(corpsenum).lock();
            corpse.Color(_color);
        ...
        if (deathMessage.length() > 0) Announce.AddMsg(deathMessage, (factionPtr.IsFriendsWith(PLAYERFACTION) ? TCODColor.red : TCODColor.brass), Position());
...
std.list < StatusEffect > * NPC.StatusEffects() {
            ...
            if (attacki.Cooldown() <= 0) {
                ...
                if (npc && !careful && effectiveStats[STRENGTH] >= npc.effectiveStats[NPCSIZE]) {
                    if (attack.Type() == DAMAGE_BLUNT || Random.Generate(4) == 0) {
                        Coordinate tar;
                        tar.X((npc.Position().X() - Position().X()) * Math.max((effectiveStats[STRENGTH] - npc.effectiveStats[NPCSIZE]) / 2, 1));
                        tar.Y((npc.Position().Y() - Position().Y()) * Math.max((effectiveStats[STRENGTH] - npc.effectiveStats[NPCSIZE]) / 2, 1));
    ...
    if (boost.shared_ptr < Entity > targetEntity = target.lock()) {
        for (std.list < Attack > .iterator attacki = attacks.begin(); attacki != attacks.end(); ++attacki) {
            if (attacki.IsProjectileMagic()) {
                if (attacki.Cooldown() <= 0) {
                    attacki.ResetCooldown();
                    boost.shared_ptr < Spell > spell = Game.CreateSpell(Position(), attacki.Projectile());
                    if (spell) {
    ...
    if (damage > 0) {
        ...
        if (res == PHYSICAL_RES && Random.Generate(99) > effectiveResistances[BLEEDING_RES]) {
            ...
            if (Random.Generate(10) == 0 && attack.Type() == DAMAGE_SLASH || attack.Type() == DAMAGE_PIERCE) {
                int gibId = Game.CreateItem(Position(), Item.StringToItemType("Gib"), false, -1);
                boost.shared_ptr < Item > gib = Game.GetItem(gibId).lock();
                if (gib) {
...
boost.weak_ptr < Squad > NPC.MemberOf() const {
    return squad;
}
...
void NPC.GetMainHandAttack(Attack & attack) {
    ...
    if (boost.shared_ptr < Item > weapon = mainHand.lock()) {
        ...
        for (std.vector < std.pair < StatusEffectType, int > > .iterator effecti = wAttack.StatusEffects().begin(); effecti != wAttack.StatusEffects().end(); ++effecti) {
...
void NPC.FindNewWeapon() {
    ...
    boost.weak_ptr < Item > newWeapon = Game.FindItemByCategoryFromStockpiles(weaponCategory, Position(), BETTERTHAN, weaponValue);
...
void NPC.FindNewArmor() {
    ...
    boost.weak_ptr < Item > newArmor = Game.FindItemByCategoryFromStockpiles(armorCategory, Position(), BETTERTHAN, armorValue);
        ...
        for (int endy = low.Y(); endy < high.Y(); endy += 2) {
            ...
            if (end.onRectangleEdges(low, high)) {
                ...
                do {
                    ...
                    if (constructUid >= 0) {
                        nearConstructions.push_back(Game.GetConstruction(constructUid));
                    }
                    ...
                    for (std.set < int > .iterator npci = map.NPCList(p).begin(); npci != map.NPCList(p).end(); ++npci) {
                        if ( * npci != uid) {
                            boost.shared_ptr < NPC > npc = Game.GetNPC( * npci);
...
bool NPC.HasTrait(Trait trait) const {
...
void NPC.ApplyEffects(boost.shared_ptr < Item > item) {
    if (item) {
        for (std.vector < std.pair < StatusEffectType, int > > .iterator addEffecti = Item.Presets[item.Type()].addsEffects.begin(); addEffecti != Item.Presets[item.Type()].addsEffects.end(); ++addEffecti) {
            if (Random.Generate(99) < addEffecti.second)
                TransmitEffect(addEffecti.first);
        }
        for (std.vector < std.pair < StatusEffectType, int > > .iterator remEffecti = Item.Presets[item.Type()].removesEffects.begin(); remEffecti != Item.Presets[item.Type()].removesEffects.end(); ++remEffecti) {
...
void NPC.UpdateHealth() {
    ...
    if (faction == PLAYERFACTION && health < maxHealth / 2 && !HasEffect(HEALING)) {
        ...
        if (!healJobFound && Item.GoodEffectAdders.find(HEALING) != Item.GoodEffectAdders.end()) {
            ...
            for (std.multimap < StatusEffectType, ItemType > .iterator fixi = Item.GoodEffectAdders.equal_range(HEALING).first; fixi != Item.GoodEffectAdders.equal_range(HEALING).second && !healItem; ++fixi) {
                healItem = Game.FindItemByTypeFromStockpiles(fixi.second, Position()).lock();
            }
            if (healItem) {
...
void NPC.save(OutputArchive & ar,
