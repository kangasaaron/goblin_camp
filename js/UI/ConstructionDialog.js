...
export class ConstructionDialog extends UIContainer {
  ...
  static ConstructionInfoDialog (wcons) {
    ...
    if (cons) {
      ...
      if (!this.constructionInfoDialog) {
        ...
        const dialog = new ConstructionDialog(50, 5)
        ...
        if (cons.Producer()) {
          ...
          const productList = new ProductList(cons)
  ...
  Rename () {
    if (this.construct.lock()) {
      const contents = new UIContainer([], 1, 1, 28, 7)
      ...
      const renameDialog = new Dialog(contents, 'Rename', 30, 8)
  ...
  Expand () {
    if (this.construct.lock()) {
      const rectCall = Stockpile.Expand.bind(this, this.construct.lock())
      const placement = this.Game.i.CheckPlacement.bind(this, Coordinate(1, 1),
  ...
  CancelJob (job) {
    const cons = this.construct.lock()
    if (cons) {
      cons.CancelJob(job)
    }
  }
