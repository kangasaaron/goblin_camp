...
void SpawningPool.*/
    ...
    Update() {
        if (condition > 0) {
            ...
            if (jobCount < 4) {
                if (dumpFilth && Random.Generate(UPDATES_PER_SECOND * 4) == 0) {
                    if (Game.filthList.size() > 0) {
                        ...
                        let filthLocation = Game.FindFilth(Position());
                ...
                if (dumpCorpses && StockManager.CategoryQuantity(Item.StringToItemCategory("Corpse")) > 0 &&
                    ...
                    corpseDumpJob.tasks.push_back(Task(FIND, Position(), null, Item.StringToItemCategory("Corpse")));
            ...
            while (!corpseContainer.empty()) {
                ...
                for (let i = 0; i < Random.Generate(1, 2); ++i)
    ...
    Expand(message = true) {
            ...
            if (location != undefinedCoordinate) {
                ...
                if (map.GetConstruction(location) >= 0) {
                    let construct;
                    if (construct = Game.GetConstruction(map.GetConstruction(location)).lock()) {
    ...
    Spawn() {
        ...
        if (spawnLocation != undefined) {
            ...
            let goblinRatio = static_cast < float > (Game.GoblinCount()) / Game.OrcCount();
            let goblin = false;
            let orc = false;
