...
export class Serialization {
	WritePayload(ofs) {
		...
		oarch.WriteBuffer(Game.Inst().serialize());
		oarch.WriteBuffer(JobManager.Inst().seralize());
		oarch.WriteBuffer(Camp.Inst().seralize());
		oarch.WriteBuffer(StockManager.Inst().seralize());
		oarch.WriteBuffer(Map.Inst().seralize());
	}

	ReadPayload(ifs) {
		let iarch = ifs;
		Entity.uids = iarch.ReadUInt32();
		Game.Inst().deserialize(iarch);
		JobManager.Inst().deserialize(iarch);
		Camp.Inst().deserialize(iarch);
		StockManager.Inst().deserialize(iarch);
		Map.Inst().deserialize(iarch);
	}
