...
class TCODMapRenderer extends /*public*/ MapRenderer {
    //public:
    ...
    void DrawCursor(const Coordinate & start,
        const Coordinate & end, bool placeable);

    void SetTranslucentUI(bool);

    //private:
    TCODConsole * the_console;
    int cursorChar;
    Coordinate upleft;
};
/* Copyright 2010-2011 Ilkka Halila
...
void TCODMapRenderer.DrawMap(Map * map, float focusX, float focusY, int viewportX, int viewportY, int viewportW, int viewportH) {
    ...
    if (viewportW == -1) {
        viewportW = the_console.getWidth();
    } else {
        viewportW /= charX;
    }
    if (viewportH == -1) {
        viewportH = the_console.getHeight();
    } else {
        viewportH /= charY;
    }
    viewportX /= charX;
        ...
        for (int x = upleft.X(); x < upleft.X() + minimap.getWidth(); ++x) {
            ...
            if (map.IsInside(xy)) {
                ...
                if (!(map.GetOverlayFlags() & TERRAIN_OVERLAY)) {
                    ...
                    if (natNum >= 0) {
                        Game.natureList[natNum].Draw(upleft, & minimap);
                    }
                }
    ...
    if (!(map.GetOverlayFlags() & TERRAIN_OVERLAY)) {
        InternalDrawMapItems("static constructions", Game.staticConstructionList, upleft, & minimap);
        InternalDrawMapItems("dynamic constructions", Game.dynamicConstructionList, upleft, & minimap);
        //TODO: Make this consistent
        for (std.map < int, boost.shared_ptr < Item > > .iterator itemi = Game.itemList.begin(); itemi != Game.itemList.end();) {
            if (!itemi.second) {
                std.map < int, boost.shared_ptr < Item > > .iterator tmp = itemi;
                ++itemi;
                Game.itemList.erase(tmp);
                continue;
    ...
    InternalDrawMapItems("NPCs", Game.npcList, upleft, & minimap);
    ...
    TCODConsole.blit( & minimap, 0, 0, viewportW, viewportH, the_console, viewportX, viewportY);
}
...
void TCODMapRenderer.DrawCursor(const Coordinate & start,
    const Coordinate & end, bool placeable) {
...
void TCODMapRenderer.DrawCursor(const Coordinate & pos, bool placeable) {
    if (pos.X() - upleft.X() >= 0 && pos.X() - upleft.X() < the_console.getWidth() &&
        pos.Y() - upleft.Y() >= 0 && pos.Y() - upleft.Y() < the_console.getHeight()) {
        if (!placeable) the_console.putCharEx(pos.X() - upleft.X(), pos.Y() - upleft.Y(), cursorChar, TCODColor.red, TCODColor.black);
        else the_console.putCharEx(pos.X() - upleft.X(), pos.Y() - upleft.Y(), cursorChar, TCODColor.green, TCODColor.black);
    }
}
