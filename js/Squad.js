	...
	*/
	...
	Rearm() {
		for (let memberi of this.members) {
			let npc = Game.Inst().GetNPC(memberi);
			if (npc)
				npc.FindNewWeapon();
		}
	}
	...
	RemoveAllMembers() {
		for (let membi of this.members) {
			let npc = Game.Inst().GetNPC(membi);
			if (npc)
				npc.MemberOf(null);
		}
		this.members.clear();
	...
	UpdateMembers() {
		if (this.members.length < this.memberReq) {
			let newMember = Game.Inst().FindMilitaryRecruit();
			if (newMember >= 0) {
				let npc = Game.Inst().GetNPC(newMember);
				if (npc) {
			...
			let npc = Game.Inst().GetNPC(this.members[this.members.length - 1]);
			if (npc)
				npc.MemberOf(null);
			this.members.pop();
		}
