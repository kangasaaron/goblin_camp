...
export class Trap extends Construction {
	...
	Update() {
		...
		let npc = Game.Inst().GetNPC(this.map.NPCList(this.pos)[0]);
	...
	save(ar, version) {
		...
		ar.save(this, "ready");
		ar.save(this, "reloadJob");
		ar.save(this, "readyGraphic");
	}

	load(ar, version) {
		super.load(ar, version);
		this.ready = ar.load(this, "ready");
		this.reloadJob = ar.load(this, "reloadJob");
		this.readyGraphic = ar.load(this, "readyGraphic");
	}
}