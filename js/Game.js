...
import "boost/thread.js"
import "boost/date_time/posix_time/posix_time_duration.js"
...
int Game.PlaceConstruction(Coordinate target, ConstructionType construct) {
    ...
    for (std.list < ItemCategory > .iterator mati = Construction.Presets[construct].materials.begin(); mati != Construction.Presets[construct].materials.end(); ++mati) {
        boost.weak_ptr < Item > material = Game.FindItemByCategoryFromStockpiles( * mati, target, EMPTY);
        if (boost.shared_ptr < Item > item = material.lock()) {
            ...
            componentList.clear();
            Announce.AddMsg((boost.format("Cancelled %s: insufficient [%s] in stockpiles") % Construction.Presets[construct].name % Item.ItemCategoryToString( * mati)).str(), TCODColor.red);
            return -1;
        }
    }
    ...
    if (Construction.AllowedAmount[construct] >= 0) {
        if (Construction.AllowedAmount[construct] == 0) {
            Announce.AddMsg("Cannot build another " + Construction.Presets[construct].name + "!", TCODColor.red);
            return -1;
        }
    ...
    if (Construction.Presets[construct].dynamic) {
        Game.dynamicConstructionList.insert(std.pair < int, boost.shared_ptr < Construction > > (newCons.Uid(), newCons));
    } else {
        Game.staticConstructionList.insert(std.pair < int, boost.shared_ptr < Construction > > (newCons.Uid(), newCons));
    }
...
int Game.PlaceStockpile(Coordinate a, Coordinate b, ConstructionType stockpile, int symbol) {
    ...
    if (Construction.Presets[stockpile].dynamic) {
        Game.dynamicConstructionList.insert(std.pair < int, boost.shared_ptr < Construction > > (newSp.Uid(), static_cast < boost.shared_ptr < Construction > > (newSp)));
    } else {
        Game.staticConstructionList.insert(std.pair < int, boost.shared_ptr < Construction > > (newSp.Uid(), static_cast < boost.shared_ptr < Construction > > (newSp)));
    }
...
void Game.DismantleConstruction(Coordinate a, Coordinate b) {
    for (int x = a.X(); x <= b.X(); ++x) {
        for (int y = a.Y(); y <= b.Y(); ++y) {
            Coordinate p(x, y);
            int construction = Map.GetConstruction(p);
            if (construction >= 0) {
...
void Game.CreateWater(Coordinate pos, int amount, int time) {
    ...
    boost.shared_ptr < FilthNode > filth = Map.GetFilth(pos).lock();

    boost.weak_ptr < WaterNode > water(Map.GetWater(pos));
    if (!water.lock()) {
...
void Game.CreateWaterFromNode(boost.shared_ptr < WaterNode > water) {
    if (water) {
        boost.shared_ptr < FilthNode > filth = Map.GetFilth(water.Position()).lock();
        boost.weak_ptr < WaterNode > existingWater(Map.GetWater(water.Position()));
...
Coordinate Game.FindFilth(Coordinate pos) {
    ...
    if (pos.X() >= 0) {
        for (int i = 0; i < 10; ++i) {
            ...
            boost.shared_ptr < FilthNode > filth = Map.GetFilth(candidate).lock();
    ...
    for (int i = 0; i < 10; ++i) {
        Coordinate candidate = Random.ChooseInRadius(Camp.Center(), 5);
        boost.shared_ptr < FilthNode > filth = Map.GetFilth(candidate).lock();
        if (filth && filth.Depth() > 0 && Map.IsWalkable(candidate))
            return candidate;
    }
...
void Game.Update() {
            ...
            case EarlySpring:
                ...
                if (Config.GetCVar < bool > ("autosave")) {
                    ...
                    if (Data.SaveGame(saveName, false))
                        ...
                        Announce.AddMsg("Failed to autosave! Refer to the logfile", TCODColor.red);
                }
            case Spring:
            case LateSpring:
                SpawnTillageJobs();
    ...
    for (std.list < boost.weak_ptr < FireNode > > .iterator fireit = fireList.begin(); fireit != fireList.end();) {
        if (boost.shared_ptr < FireNode > fire = fireit.lock()) {
            ...
            if (fire.GetHeat() <= 0) {
                Map.SetFire(fire.Position(), boost.shared_ptr < FireNode > ());
        ...
        if ((!reserveItem || !item.Reserved()) && item.GetFaction() == PLAYERFACTION) {
            ...
            if (nearest) {
                ...
                if (item.IsCategory(Item.StringToItemCategory("Food"))) priority = HIGH;
                    ...
                    float stockDeficit = (float) StockManager.TypeQuantity(itemType) / (float) StockManager.Minimum(itemType);
...
void Game.FellTree(Coordinate a, Coordinate b) {
    for (int x = a.X(); x <= b.X(); ++x) {
        for (int y = a.Y(); y <= b.Y(); ++y) {
            int natUid = Map.GetNatureObject(Coordinate(x, y));
            if (natUid >= 0) {
                boost.shared_ptr < NatureObject > natObj = Game.natureList[natUid];
...
void Game.Undesignate(Coordinate a, Coordinate b) {
    for (int x = a.X(); x <= b.X(); ++x) {
        for (int y = a.Y(); y <= b.Y(); ++y) {
            Coordinate p(x, y);
            int natUid = Map.GetNatureObject(p);
            if (natUid >= 0) {
                boost.weak_ptr < NatureObject > natObj = Game.natureList[natUid];
...
void Game.DecayItems() {
    ...
    for (std.list < boost.weak_ptr < BloodNode > > .iterator bli = bloodList.begin(); bli != bloodList.end();) {
        if (boost.shared_ptr < BloodNode > blood = bli.lock()) {
            ...
            if (blood.Depth() <= 0) {
                Map.SetBlood(blood.Position(), boost.shared_ptr < BloodNode > ());
...
void Game.CreateFilth(Coordinate pos, int amount) {
    ...
    if (Map.IsInside(pos)) {
        ...
        while (amount > 0 && loops < 1000) {
            ...
            boost.weak_ptr < FilthNode > filth(Map.GetFilth(pos));
...
void Game.SetSquadTargetCoordinate(Order order, Coordinate target, boost.shared_ptr < Squad > squad, bool autoClose) {
    ...
    Announce.AddMsg((boost.format("[%1%] guarding position (%2%,%3%)") % squad.Name() % target.X() % target.Y()).str(), TCODColor.white, target);
    Map.AddMarker(MapMarker(FLASHINGMARKER, 'X', target, UPDATES_PER_SECOND * 5, TCODColor.azure));
}
void Game.SetSquadTargetEntity(Order order, Coordinate target, boost.shared_ptr < Squad > squad) {
    if (Map.IsInside(target)) {
        std.set < int > * npcList = Map.NPCList(target);
        if (!npcList.empty()) {
            squad.AddOrder(order);
            squad.AddTargetEntity(Game.npcList[ * npcList.begin()]);
            UI.CloseMenu();
            Announce.AddMsg((boost.format("[%1%] following %2%") % squad.Name() % Game.npcList[ * npcList.begin()].Name()).str(), TCODColor.white, target);
        }
    }
}
...
void Game.MoveCam(float x, float y) {
    camX = Math.min(Math.max(x * renderer.ScrollRate() + camX, 0.0 f), Map.Width() + 1.0 f);
    camY = Math.min(Math.max(y * renderer.ScrollRate() + camY, 0.0 f), Map.Height() + 1.0 f);
}
    ...
    if (Map.IsWalkable(pos) && (Map.GetType(pos) == TILEGRASS || Map.GetType(pos) == TILESNOW) && Random.Generate(4) < 2) {
        ...
        if (Random.Generate(50) < rarity) {
            for (int clus = 0; clus < NatureObject.Presets[chosen].cluster; ++clus) {
                Coordinate a = Map.Shrink(Random.ChooseInRadius(pos, clus));
...
void Game.GatherItems(Coordinate a, Coordinate b) {
    for (int x = a.X(); x <= b.X(); ++x) {
        for (int y = a.Y(); y <= b.Y(); ++y) {
            Coordinate p(x, y);
            if (Map.IsInside(p)) {
                for (std.set < int > .iterator itemuid = Map.ItemList(p).begin(); itemuid != Map.ItemList(p).end(); ++itemuid) {
...
void Game.RemoveFilth(Coordinate pos) {
    ...
    if (filth) {
        ...
        Map.SetFilth(pos, boost.shared_ptr < FilthNode > ());
    }
}
...
void Game.RemoveWater(Coordinate pos, bool removeFromList) {
    ...
    if (water) {
        ...
        Map.SetWater(pos, boost.shared_ptr < WaterNode > ());
...
void Game.Damage(Coordinate pos) {
    ...
    boost.shared_ptr < Construction > construction = GetConstruction(Map.GetConstruction(pos)).lock();
    if (construction) {
        construction.Damage( & attack);
    }
    for (std.set < int > .iterator npcuid = Map.NPCList(pos).begin(); npcuid != Map.NPCList(pos).end(); ++npcuid) {
...
void Game.CreateFire(Coordinate pos, int temperature) {
    if (fireList.empty()) {
        Announce.AddMsg("Fire!", TCODColor.red, pos);
        if (Config.GetCVar < bool > ("pauseOnDanger"))
            Game.AddDelay(UPDATES_PER_SECOND, boost.bind( & Game.Pause, Game.Inst()));
    }

    boost.weak_ptr < FireNode > fire(Map.GetFire(pos));
...
void Game.Hungerize(Coordinate pos) {
    if (Map.IsInside(pos)) {
        for (std.set < int > .iterator npci = Map.NPCList(pos).begin(); npci != Map.NPCList(pos).end(); ++npci) {
            boost.shared_ptr < NPC > npc;
...
void Game.DisplayStats() {
    ...
    Label * points = new Label((boost.format("Points: %d") % Stats.GetPoints()).str(), 1, 2, TCOD_LEFT);
    ...
    Label * burntItems = new Label((boost.format("Burnt items: %d") % Stats.GetItemsBurned()).str(), 1, 9, TCOD_LEFT);
    ...
    productionFrame.AddComponent(new ScrollPanel(1, 2, 23, 15,
        new UIList < std.pair < std.string, unsigned > , boost.unordered_map < std.string, unsigned > > ( & Stats.itemsBuilt, 0, 0, 24, Stats.itemsBuilt.size(),
    ...
    productionFrame.AddComponent(new ScrollPanel(1, 18, 23, 15,
        new UIList < std.pair < std.string, unsigned > , boost.unordered_map < std.string, unsigned > > ( & Stats.constructionsBuilt, 0, 0, 24, Stats.constructionsBuilt.size(),
            boost.bind(DrawText, _1, _2, _3, _4, _5, _6, _7), 0, false, 0)));
    ...
    deathFrame.AddComponent(new ScrollPanel(1, 1, 23, 32,
        new UIList < std.pair < std.string, unsigned > , boost.unordered_map < std.string, unsigned > > ( & Stats.deaths, 0, 0, 24, Stats.deaths.size(),
