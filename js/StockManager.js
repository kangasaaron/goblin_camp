        ...
        if (!producerFound) { //Haven't found a producer, so check NatureObjects if a tree has this item as a component
            for (size_t natObj = 0; natObj < NatureObject.Presets.size(); ++natObj) {
                if (NatureObject.Presets[natObj].tree) {
                    for (std.list < ItemType > .iterator compi = NatureObject.Presets[natObj].components.begin(); compi != NatureObject.Presets[natObj].components.end(); ++compi) {
                        if ( * compi == item) {
                            producables.insert(item);
    ...
    for (ItemType type = 0; type < static_cast < int > (Item.Presets.size()); ++type) {
        ...
        if (producables.find(type) != producables.end()) {
            if (minimums[type] > 0 && difference > 0) { //Only consider production if we have a positive minimum
                ...
                if (fromTrees.find(type) != fromTrees.end()) { //Item is a component of trees
                    ...
                    for (std.list < boost.weak_ptr < NatureObject > > .iterator treei = designatedTrees.begin(); treei != designatedTrees.end() && difference > 0; ++treei) {
                        if (!treei.lock()) continue;

                        bool componentInTree = false;
                        for (std.list < ItemType > .iterator compi = NatureObject.Presets[treei.lock().Type()].components.begin(); compi != NatureObject.Presets[treei.lock().Type()].components.end(); ++compi) {
                            if ( * compi == type) {
                    ...
                    if (difference > 0) {
                        Coordinate waterLocation = Game.FindWater(Camp.Center());
        ...
        if (dumpables.find(type) != dumpables.end() &&
            ...
            if (Random.Generate(59) == 0) {
                if (boost.shared_ptr < SpawningPool > spawningPool = Camp.spawningPool.lock()) {
                    boost.shared_ptr < Job > dumpJob(new Job("Dump " + Item.ItemTypeToString(type), LOW));
                    boost.shared_ptr < Item > item = Game.FindItemByTypeFromStockpiles(type, spawningPool.Position()).lock();
                    if (item) {
                        dumpJob.Attempts(1);
...
void StockManager.UpdateWorkshops(boost.weak_ptr < Construction > cons, bool add) {
    if (add) {
        ...
        for (std.multimap < ConstructionType, boost.weak_ptr < Construction > > .iterator worki = workshops.begin(); worki != workshops.end(); ++worki) {
...
void StockManager.UpdateTreeDesignations(boost.weak_ptr < NatureObject > nObj, bool add) {
    if (boost.shared_ptr < NatureObject > natObj = nObj.lock()) {
        if (add) {
            ...
            for (std.list < boost.weak_ptr < NatureObject > > .iterator desi = designatedTrees.begin(); desi != designatedTrees.end();) {
...
void StockManager.save(OutputArchive & ar,
...
void StockManager.load(InputArchive & ar,
