...
class Stockpile extends /*public*/ Construction, public ContainerListener {
...
void Stockpile.SetMap(Map * map) {
    ...
    colors.insert(std.pair < Coordinate, TCODColor > (pos, TCODColor.lerp(color, map.GetColor(pos), 0.75 f)));
}
...
boost.weak_ptr < Item > Stockpile.FindItemByCategory(ItemCategory cat, int flags, int value) {
    ...
    for (std.map < Coordinate, boost.shared_ptr < Container > > .iterator conti = containers.begin(); conti != containers.end() && itemsFound < amount[cat]; ++conti) {
...
boost.weak_ptr < Item > Stockpile.FindItemByType(ItemType typeValue, int flags, int value) {
    ...
    ItemCategory cat = * Item.Presets[typeValue].categories.begin();
        ...
        for (int ix = from.X(); ix <= to.X(); ++ix) {
            for (int iy = from.Y(); iy <= to.Y(); ++iy) {
                ...
                if (map.GetConstruction(p) == -1 && map.IsBuildable(p) &&
                    ...
                    //Update color
                    colors.insert(std.pair < Coordinate, TCODColor > (p, TCODColor.lerp(color, map.GetColor(p), 0.75 f)));
                    ++expansion;
                }
            }
        }
...
void Stockpile.Draw(Coordinate upleft, TCODConsole * the_console) {
    int screenx, screeny;
    ...
    if (boost.shared_ptr < Item > item = witem.lock()) {
        ...
        if (Item.Presets[item.Type()].fitsin >= 0) {
            ...
            if (std.abs(demand[Item.Presets[item.Type()].fitsin] - lastDemandBalance[Item.Presets[item.Type()].fitsin]) > 10) {
                Game.RebalanceStockpiles(Item.Presets[item.Type()].fitsin,
...
void Stockpile.TranslateInternalContainerListeners() {
    for (std.map < Coordinate, boost.shared_ptr < Container > > .iterator it = containers.begin(); it != containers.end(); ++it) {
...
void Stockpile.Dismantle(const Coordinate & p) {
    if (!Construction.Presets[type].permanent) {
        if (p == undefined) {
            ...
            Stockpile.Erase(p);
            if (containers.empty()) Game.RemoveConstruction(boost.static_pointer_cast < Construction > (shared_from_this()));
        }
    }
}
...
void Stockpile.Reorganize() {
    for (std.map < Coordinate, boost.shared_ptr < Container > > .const_iterator space = containers.begin(); space != containers.end(); ++space) {
...
void Stockpile.save(OutputArchive & ar,
...
void Stockpile.load(InputArchive & ar,
