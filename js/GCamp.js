...
void MainLoop() {
    Game * game = Game.Inst();
    if (!game.Running()) {
        Announce.AddMsg("Press 'h' for keyboard shortcuts", TCODColor.cyan);
    }
    game.Running(true);
...
void StartNewGame() {
    ...
    for (int tries = 0; tries < 20; ++tries) {
        std.pair < int, Coordinate > candidate(0, Random.ChooseInExtent(zero + 100, Map.Extent() - 100));
...
int MainMenu() {
    ...
    bool exit = false;
    int width = 20;
    int edgex = Game.ScreenWidth() / 2 - width / 2;
    int height = (entryCount * 2) + 2;
    int edgey = Game.ScreenHeight() / 2 - height / 2;
    int selected = -1;
...
void SettingsMenu() {
    ...
    const int w = 40;
    const int h = 29;
    const int x = Game.ScreenWidth() / 2 - (w / 2);
    const int y = Game.ScreenHeight() / 2 - (h / 2);
...
void ModsMenu() {
    ...
    TCODConsole sub(w - 2, Math.max(1, subH));
...
void TilesetsMenu() {
    ...
    TCODConsole sub(listWidth - 2, Math.max(1, subH));
...
void KeysMenu() {
    ...
    BOOST_FOREACH(Config.KeyMap.value_type pair, keyMap) {
        w = Math.max(w, (int) pair.first.size() + 7); // 2 for borders, 5 for [ X ]
