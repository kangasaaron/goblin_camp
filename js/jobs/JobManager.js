...
import { TCODColor, TCODConsole } from "../../fakeTCOD/libtcod.js";
import { Constants } from "../Constants.js";
import { FindBestMatching } from "../FindBestMatching.js";
...
export class JobManager extends Serializable {
    ...
    constructor() {        
        ...
        /** @type {Array<Job>} */
        this.availableList = new Array(JobPriority.PRIORITY_COUNT);
        /** @type {Array<Job>} */
        this.waitingList = [];
        /** @type {Array<number>} */
    ...
    Draw(pos, from = 0, width = 40, height = 40, the_console = TCODConsole.root) {
        ...
        for (let i = 0; i <= JobPriority.JobPriority.PRIORITY_COUNT; i++) {
            the_console.setDefaultForeground(color_mappings[i]);
            for (let jobi of (i < JobPriority.JobPriority.PRIORITY_COUNT ? this.availableList[i] : this.waitingList)) {
                if (skip < from) {
                    ++skip;
                    continue;
                }

                npc = Game.i.GetNPC(jobi.Assigned());
                if (npc) {
    ...
    AssignJobs() {
        ...
        for (let i = 0; i < Item.Categories.length; ++i) {
            maxToolJobs[i] = StockManager.CategoryQuantity(new ItemCat(i)) - this.toolJobs[i].length;
        }

        for (let i = 0; i < JobPriority.JobPriority.PRIORITY_COUNT && (this.expertNPCsWaiting.length || this.menialNPCsWaiting.length); i++) {
            ...
            let menialMatrixSize = Math.max(menialJobsToAssign.length, this.menialNPCsWaiting.length);
            let expertMatrixSize = Math.max(expertJobsToAssign.length, this.expertNPCsWaiting.length);
            /** @type {boost.numeric.ublas.matrix} */
            let menialMatrix = new Array(menialMatrixSize);
            menialMatrix = menialMatrix.forEach(() => new Array(menialMatrixSize).fill(0));
            /** @type {boost.numeric.ublas.matrix} */
            let expertMatrix = new Array(expertMatrixSize);
            expertMatrix = expertMatrix.forEach(() => new Array(expertMatrixSize).fill(0));
            ...
            for (let x = 0; x < menialMatrixSize; x++) {
                for (let y = 0; y < menialMatrixSize; y++) {
                    ...
                    let npc = Game.i.GetNPC(this.menialNPCsWaiting[x]);
            ...
            for (let x = 0; x < expertMatrixSize; x++) {
                for (let y = 0; y < expertMatrixSize; y++) {
                    if (x >= this.expertNPCsWaiting.length || y >= expertJobsToAssign.length) {
                        ...
                        let npc = Game.i.GetNPC(this.expertNPCsWaiting[x]);
...
JobManager.CLASS_VERSION = 1;

Singletonify(JobManager);
