...
export class ConstructionListener extends PresetParser {
    ...
    parserProperty(name, value) {
        ...
        if (strName === "graphicLength") {
            ...
            preset.spawnFrequency = value * Constants.UPDATES_PER_SECOND;
        } else if (strName === "col") {
            ...
            while (desc.length() > 0) {
                tokens.push(desc.substr(0, desc.find_first_of(" ")));
    ...
    parserEndStruct(/*data*/) {
        ...
        if (preset.tileReqs.empty()) {
            preset.tileReqs.insert(TileType.TILEGRASS);
            preset.tileReqs.insert(TileType.TILEMUD);
            preset.tileReqs.insert(TileType.TILEROCK);
            preset.tileReqs.insert(TileType.TILESNOW);
        }
