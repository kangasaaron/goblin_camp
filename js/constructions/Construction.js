...
class ConstructionType extends Number{
    constructor(value){
        super(value)
        this.value = value
    }
...
export class Construction extends Entity {
    ...
    constructor(vtype = new ConstructionType(0), target = Coordinate.zero) {
        ...
        */
        ...
        if (!preset.color.equals(TCODColor.black))
    ...
    destructor() {
        ...
        if (this.built) {
            if (!this.HasTag(this.CENTERSCAMP))
                Camp.i.UpdateCenter(this.Center(), false);
            else
                Camp.i.UnlockCenter();
        }
    }
    ...
    SpawnProductionJob() {
        ...
        for (let compi = 0; compi < Item.Components(this.jobList[0]).length; ++compi) {
            let item = Game.i.FindItemByCategoryFromStockpiles(Item.Components(this.jobList[0], compi), this.Center(), APPLYMINIMUMS);
            if (item.lock()) {
    ...
    Draw(upleft, the_console) {
        ...
        for (let i = 1; i < this.graphic.length; ++i) {
            if (screenx + i - 1 >= 0 && screeny >= 0 && screenx + i - 1 < the_console.getWidth() && screeny < the_console.getHeight()) {
                if (this.dismantle) the_console.setCharBackground(screenx + i - 1, screeny, TCODColor.darkGrey);
                else the_console.setCharBackground(screenx + i - 1, screeny, new TCODColor(Math.round(50 - Math.cos(strobe) * 50), Math.round(50 - Math.cos(strobe) * 50), Math.round(50 - cos(strobe) * 50)));
        ...
        for (let itemi of this.materialsUsed.GetItems()) {
            ...
            if (item = itemi.lock()) {
                ...
                if (item.Type() !== Item.StringToItemType("debris"))
                    item.SetFaction(Constants.PLAYERFACTION); //Return item to player faction
            }
        }
        while (!this.materialsUsed.empty()) {
            this.materialsUsed.RemoveItem(this.materialsUsed.GetFirstItem());
    ...
    SpawnRepairJob() {
        ...
        let repairItem = Game.i.FindItemByCategoryFromStockpiles(cat, this.Position()).lock();
    ...
    Damage(attack) {
        ...
        let damage = (Game.i.DiceToInt(attack.Amount()) * damageModifier);
        this.condition -= damage;

        if (attack.Type() === DamageType.DAMAGE_FIRE && Random.i.Generate(5) === 0) {
    ...
    UpdateWallGraphic(recurse = true, updateSelf = true) {
        ...
        for (i = 0; i < 4; ++i) {
            ...
            let cons = Game.i.GetConstruction(consId[i]).lock();
        ...
        if (recurse) {
            for (let i = 0; i < 4; ++i) {
                if (wod[i])
                    Game.i.GetConstruction(consId[i]).lock().UpdateWallGraphic(false);
            }
        }
    }
    UpdateSpawnCreatures() {
        if (Random.i.Generate(this.preset.spawnFrequency - 1) !== 0) return;
        let monsterType = Game.i.GetRandomNPCTypeByTag(this.preset.spawnCreaturesTag);
        let friendly = NPC.Presets[monsterType].tags.has("friendly");
        let announceColor = friendly ? TCODColor.green : TCODColor.red;

        if (!friendly && Config.i.GetCVar("pauseOnDanger"))
            Game.i.AddDelay(Constants.UPDATES_PER_SECOND, Game.i.Pause.bind(Game.i));

        let amount = Game.i.DiceToInt(NPC.Presets[monsterType].group);
        if (amount === 1) {
    ...
    Build() {
        ...
        for (let itemi of this.materialsUsed.GetItems()) {
            ...
            if (Random.i.Generate(9) < 8) { //80% of materials can't be recovered
        ...
        if (!this.HasTag(ConstructionTag.CENTERSCAMP))
            Camp.i.UpdateCenter(this.Center(), true);
        else
            Camp.i.LockCenter(this.Center());
        Camp.i.ConstructionBuilt(this.type);
        Stats.i.ConstructionBuilt(this.preset.name);

        Script.i.Event.BuildingCreated(this, pos.X(), pos.Y());
    ...
    complete() {
        ...
        for (let i = 0; i < Item.Presets[this.jobList[0]].multiplier; ++i) {
            if (itemContainer)
                Game.i.CreateItem(this.Position() + this.preset.productionSpot, this.jobList[0], false, 0, components, itemContainer);
            else
                Game.i.CreateItem(this.Position() + this.preset.productionSpot, this.jobList[0], true, 0, components);
