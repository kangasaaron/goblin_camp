...
Construction.~Construction() {
    ...
    if (producer) StockManager.UpdateWorkshops(boost.weak_ptr < Construction > (), false);
    ...
    if (built) {
        if (!HasTag(CENTERSCAMP)) Camp.UpdateCenter(Center(), false);
        else Camp.UnlockCenter();
    }
}
...
int Construction.Build() {
    ...
    if (condition > 0) {
        ...
        for (std.list < boost.weak_ptr < Item > > .iterator itemi = itemsToRemove.begin(); itemi != itemsToRemove.end(); ++itemi) {
            ...
            Game.CreateItem(materialsUsed.Position(), Item.StringToItemType("debris"), false, -1,
        ...
        if (producer) {
            StockManager.UpdateWorkshops(boost.static_pointer_cast < Construction > (shared_from_this()), true);
            for (unsigned int prod = 0; prod < Construction.Presets[type].products.size(); ++prod) {
                StockManager.UpdateQuantity(Construction.Presets[type].products[prod], 0);
            }
        }
        built = true;
        if (!HasTag(CENTERSCAMP)) Camp.UpdateCenter(Center(), true);
        else Camp.LockCenter(Center());
        Camp.ConstructionBuilt(type);
        Stats.ConstructionBuilt(Construction.Presets[type].name);
    ...
    if (jobList.size() > 0) {
        ...
        if (smoke == 2 && Construction.Presets[type].chimney != undefined) {
            if (Random.Generate(9) == 0) {
                boost.shared_ptr < Spell > smoke = Game.CreateSpell(Position() + Construction.Presets[type].chimney, Spell.StringToSpellType("smoke"));
                ...
                if (Random.Generate(50000) == 0) {
                    boost.shared_ptr < Spell > spark = Game.CreateSpell(pos, Spell.StringToSpellType("spark"));
        ...
        if (progress >= 100) {
            ...
            for (int i = 0; i < Item.Presets[jobList[0]].multiplier; ++i) {
                if (itemContainer) Game.CreateItem(Position() + Construction.Presets[type].productionSpot, jobList[0], false, 0, components, itemContainer);
                else Game.CreateItem(Position() + Construction.Presets[type].productionSpot, jobList[0], true, 0, components);
                Stats.ItemBuilt(Item.Presets[jobList[0]].name);
            }
...
bool Construction.SpawnProductionJob() {
    ...
    if (!reserved) {
        ...
        for (int compi = 0; compi < (signed int) Item.Components(jobList.front()).size(); ++compi) {
            boost.weak_ptr < Item > item = Game.FindItemByCategoryFromStockpiles(Item.Components(jobList.front(), compi), Center(), APPLYMINIMUMS);
            if (item.lock()) {
...
void Construction.UpdateWallGraphic(bool recurse, bool self) {
    ...
    for (int i = 0; i < 4; ++i) {
        ...
        wod[i] = false;
        if (consId[i] > -1) {
            boost.shared_ptr < Construction > cons = Game.GetConstruction(consId[i]).lock();
...
void Construction.Update() {
    if (Construction.Presets[type].spawnCreaturesTag != "" && condition > 0) {
        if (Random.Generate(Construction.Presets[type].spawnFrequency - 1) == 0) {
            NPCType monsterType = Game.GetRandomNPCTypeByTag(Construction.Presets[type].spawnCreaturesTag);
            ...
            if (amount == 1) {
                Announce.AddMsg("A " + NPC.NPCTypeToString(monsterType) + " emerges from the " + name + "!", announceColor, Position());
            } else {
                Announce.AddMsg(NPC.Presets[monsterType].plural + " emerge from the " + name + "!", announceColor, Position());
            }
            for (int i = 0; i < amount; ++i) {
                Game.CreateNPC(Position() + ProductionSpot(type), monsterType);
            }
        }
    }

    if (!Construction.Presets[type].passiveStatusEffects.empty() && !map.NPCList(pos).empty()) {
        boost.shared_ptr < NPC > npc = Game.GetNPC( * map.NPCList(pos).begin());
...
void Construction.SpawnRepairJob() {
    if (built && condition < maxCondition && !repairJob.lock()) {
        boost.shared_ptr < Item > repairItem = Game.FindItemByCategoryFromStockpiles( * boost.next(Construction.Presets[type].materials.begin(), Random.ChooseIndex(Construction.Presets[type].materials)),
            Position()).lock();
        if (repairItem) {
